...
function Creditos({empresas, creditosData=CREDITOS_DEFAULT, onSaveCreditos, canEdit=false}) {
  ...
  const [filtPagado,setFiltPagado]=useState("Pendientes"); // "Todos" | "Pendientes" | "Pagados"
  ...
  function togglePagado(n){
    const next=creditosData.map(c=>c.n===n?{...c,pagado:!c.pagado}:c);
    if(onSaveCreditos) onSaveCreditos(next);
  }
  function eliminar(n){
                    ...
                    {canEdit&&(
                      <td style={{padding:"6px 8px",whiteSpace:"nowrap"}}>
                        <div style={{display:"flex",gap:4}}>
                          <button onClick={()=>togglePagado(c.n)} title={c.pagado?"Marcar pendiente":"Marcar pagado"}
                            style={{padding:"3px 8px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,
                              background:c.pagado?"#dcfce7":"#fef9c3",color:c.pagado?"#166534":"#854d0e"}}>
                            {c.pagado?"↩ Reabrir":"✓ Pagado"}
                          </button>
...
export default function FinanzasModule({onBack,onLogout,usuarioActual,tabPermisos={}}) {
  ...
  const empresas=useMemo(()=>{
    ...
    // Recalculate Pago Préstamos proy using current creditosData (respects pagado flag)
    Object.keys(base).forEach(empNombre=>{
      const emp = base[empNombre];
      emp.sections.forEach(sec=>{
        if(sec.cat!=="egr_nop") return;
        sec.lines.forEach(l=>{
          if(l.label==="Pago Préstamos - Total") {
            l.proy = calcPrestamosEmpresa(empNombre, creditosData);
          }
        });
      });
    });
  ...
  function applyData(d) {
    ...
    if(d?.creditos_data)  setCreditosData(prev=>d.creditos_data.length>0?d.creditos_data:prev);
  }
